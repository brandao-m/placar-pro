...
def build_match_response(match: Match, session: Session) -> MatchListRead:
    league = None
    home_team = None
    away_team = None
...
def list_matches(
    ...
    return [
        build_match_response(match=match, session=session)
...
def get_match_by_id(
    ...
    return build_match_response(match=match, session=session)